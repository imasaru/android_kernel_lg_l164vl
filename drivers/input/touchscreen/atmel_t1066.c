...
//KHG ADD
		if ((status & MXT_T100_STATUS_MASK) == MXT_T100_PRESS)
		{
        		if ((status & MXT_T100_TYPE_MASK) == MXT_T100_TYPE_FINGER)
                    ...
                    ++(data->touch_tot_cnt);
                    TOUCH_DEBUG_MSG("!!!!!!!DET <%d> FC <%d> f(%x) : SC    <%d> f(%x): TTC   <%d> FC : <%d> SC : <%d>\n",
		id, data->finger_cnt, data->finger_flag, data->stylus_cnt, data->stylus_flag, data->touch_tot_cnt, data->fc_cnt, data->sc_cnt);
		}
		else
		 {
			 if (((status & MXT_T100_TYPE_MASK) == MXT_T100_TYPE_FINGER) && (data->stylus_flag & (0x01 << id)))
			 {
				 ++(data->finger_cnt);
				 data->finger_flag |= (0x01 << id);
				 --(data->stylus_cnt);
			 ...
			 else if(((status & MXT_T100_TYPE_MASK) == MXT_T100_TYPE_STYLUS) && (data->finger_flag & (0x01 << id)))
			 {
				 --(data->finger_cnt);
				 data->finger_flag &= ~(0x01 << id);
				 ++(data->stylus_cnt);
...
//KHG ADD
             if ((data->finger_flag & (0x01 << id)) && data->finger_cnt != 0)
                ...
                TOUCH_DEBUG_MSG("!!!!!REL <%d> FC    <%d> f(%x) : SC    <%d> f(%x) : TTC   <%d> FC : <%d> SC : <%d>\n",
		id, data->finger_cnt, data->finger_flag, data->stylus_cnt, data->stylus_flag, data->touch_tot_cnt, data->fc_cnt, data->sc_cnt);
