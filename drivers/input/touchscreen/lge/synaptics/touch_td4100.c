...
/*ESD defense control for SF3*/
bool esd_defense_ctrl = true;
...
static int synaptics_change_landing_frame(struct device *dev)
{
	...
	if (synaptics_is_product(d, "PLG532", 6)) {
		/* Skip landing frame change for SF3 */
		TOUCH_I("%s : skip landing frame change\n", __func__);
		return ret;
	}
...
static int synaptics_lpwg_mode(struct device *dev)
{
	...
	if (atomic_read(&ts->state.fb) == FB_SUSPEND) {
		if (ts->role.mfts_lpwg) {
			...
			synaptics_sensing_cmd(dev, WAKEUP_SENSING);
			if (synaptics_is_product(d, "PLG635", 6))	/* Skip ForceUpdate for SF3 */
			...
			if (atomic_read(&ts->state.sleep) == IC_DEEP_SLEEP)
				...
				synaptics_esd_detect_enable(dev, 1);
	...
	if (ts->lpwg.screen) {
		...
		TOUCH_I("%s(%d) - normal\n",
				__func__, __LINE__);
		/*exception handle for fast display on/off sequence. only SF3 ESD issue*/
		if (esd_defense_ctrl == false && (atomic_read(&d->state.init) == INIT_END)) {
			TOUCH_I("defense esd packet. keep in esd disable set\n");
			synaptics_esd_detect_enable(dev, 0);
		} else {
			TOUCH_I("prepare to receive esd packets\n");
			synaptics_esd_detect_enable(dev, 1);
		}
		esd_defense_ctrl = true;
		/* End - kiwoo.han 2016-12-3 */
		synaptics_lpwg_control(dev, LPWG_NONE);
		...
		TOUCH_I("%s(%d) - normal on screen off\n",
				__func__, __LINE__);
		synaptics_esd_detect_enable(dev, 0); /* Call scene */
		synaptics_lpwg_control(dev, LPWG_NONE);
...
ESD_RECOVERY:
	...
	atomic_set(&d->state.recovery, ESD_RECOVERY_PROGRESS); /* ESD recovery operation Start Flag */
	return -ERESTART;
}
...
int synaptics_init(struct device *dev)
{
	...
	if (atomic_read(&d->state.init) == INIT_END)
		atomic_set(&d->state.init, INIT_PROGRESS);
...
static ssize_t store_finger_amplitude(struct device *dev, const char *buf, size_t count)
{
	struct synaptics_data *d = to_synaptics_data(dev);
	int value = 0;
	int ret = 0;
	...
	if (!synaptics_is_product(d, "PLG632", 6)) {
		/*Skip finger amplitude reg ctrl. only use SF3*/
		TOUCH_I("%s : skip finger amplitude reg ctrl\n", __func__);
		return ret;
	}

	ret = synaptics_read(dev, d->f12_reg.ctrl[15], buffer, sizeof(buffer));
	if (ret < 0) {
		TOUCH_E("failed to read i2c - ret:%d\n", ret);
		return ret;
	}

	/* Finger Amplitude Threshold value */
	if (value)
		buffer[0] = 0x0d;
	else
		buffer[0] = 0x33;

	ret = synaptics_write(dev, d->f12_reg.ctrl[15], buffer, sizeof(buffer));
	if (ret < 0) {
		TOUCH_E("failed to write i2c - ret:%d\n", ret);
		return ret;
	}
	...
	TOUCH_I("Finger Amplitude Threshold Buffer[0] : 0x%x\n", buffer[0]);
...
static TOUCH_ATTR(finger_amplitude, NULL, store_finger_amplitude);
...
static struct attribute *td4100_attribute_list[] = {
	...
	&touch_attr_finger_amplitude.attr,
	NULL,
};
