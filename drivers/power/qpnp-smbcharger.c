...
#define BATTCHG_PROTECT_MIN_FASTCHG_RATIO 75
static void smbchg_battchg_protect_work(struct work_struct *work)
{
	struct smbchg_chip *chip = container_of(work,
				struct smbchg_chip,
				battchg_protect_work.work);
	int min_fastchg_current_ma = chip->cfg_fastchg_current_ma * BATTCHG_PROTECT_MIN_FASTCHG_RATIO / 100;
...
static void update_vzw_chg(struct smbchg_chip *chip)
{
	...
	if (chip->usb_psy) {
		...
		if ((chip->is_floated_charger != prev_floated_charger)
			...
			pr_info("floated charger : %d\n", chip->is_floated_charger);
			pr_info("chg_present : %d\n", chip->usb_present);
			schedule_work(&chip->set_vzw_chg_work);
		} else if (prev_chg_present != chip->usb_present) {
			pr_info("chg_present : %d\n", chip->usb_present);
			schedule_work(&chip->set_vzw_chg_work);
		} else if (prev_aicl_complete != chip->aicl_complete) {
			pr_info("aicl_complete : %d\n", chip->aicl_complete);
			schedule_work(&chip->set_vzw_chg_work);
		} else if (prev_aicl_level != aicl_level) {
			pr_info("prev_aicl_level :%d, aicl_level : %d\n", 
		...
		if(chip->qc20.current_status == QC20_CURRENT_LIMMITED){
			...
			if (type == POWER_SUPPLY_TYPE_USB_DCP) {
				pr_smb(PR_LGE, "[DCP] Set ibat limit: %d, ibat thermal: %d\n",
					chip->qc20.ibat[QC20_CURRENT_LIMMITED], smbchg_ibat_limit);
				rc |= vote(chip->fcc_votable, BATT_TYPE_FCC_VOTER, true,
					chip->qc20.ibat[QC20_CURRENT_LIMMITED]);
				rc |= vote(chip->fcc_votable, THERMAL_LGE_FCC_VOTER, true,
					smbchg_ibat_limit);
...
{
	struct smbchg_chip *chip =
						container_of(work, struct smbchg_chip, set_vzw_chg_work);
	...
	pr_err("vzw aicl_complete check = %d\n", (int)chip->aicl_complete);
	aicl_level = smbchg_get_aicl_level_ma(chip);
	if ((int)chip->aicl_complete > 0) {
		if (aicl_level < VZW_SLOW_CHG_CURRENT_MIN) {
			pr_err("vzw slow charger check\n");
...
#endif
#ifdef CONFIG_LGE_PM_VZW_REQ
	INIT_WORK(&chip->set_vzw_chg_work, smbchg_set_vzw_chg_work);
#endif
#ifdef CONFIG_LGE_PM_LLK_MODE
